...
def generate_automated_audit(cv_text, jd_text, api_key, model_choice):
    ...
    # Priority list of models to try if the selected model encounters a 404
    candidate_models = [model_choice, "llama-3.1-70b-versatile", "llama-3.1-8b-instant", "llama3-70b-8192"]
    ...
    for target_model in candidate_models:
        try:
            ...
            if "404" in error_str or "model_not_found" in error_str:
                ...
                continue  # Fallback to the next candidate model
            return f"⚠️ **Groq API Error:** {error_str}"
            
    return f"⚠️ **Groq API Error (All Models Failed):** {last_error}"
...
with st.sidebar:
    ...
    selected_model = st.selectbox(
        "🤖 Select Groq Model:",
        ["llama-3.1-70b-versatile", "llama-3.1-8b-instant", "llama3-70b-8192", "mixtral-8x7b-32768"],
        index=0
    )
    
    GROQ_API_KEY = user_api_key if user_api_key.strip() else ""
